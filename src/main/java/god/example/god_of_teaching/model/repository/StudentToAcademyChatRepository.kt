...
//학원 채팅 관련 레포지토리
class StudentToAcademyChatRepository@Inject constructor(private val userDataStoreHelper : UserDataStoreHelper) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            db.collection("academyChatList").document(myUid).collection("other")
                ...
                .addOnFailureListener {
                    Log.d("버그", "StudentToAcademyChatRepository에서 채팅방 못 나가는 중 : ${it.message}")
                }
    ...
    fun getChatList(): MutableLiveData<List<ChatListInfo>> {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            db.collection("academyChatList").document(uid)
                .collection("other")
                .orderBy("timeStamp", Query.Direction.DESCENDING)
                .addSnapshotListener { snapshots, e ->
                    if (e != null) {
                        Log.w("StudentToAcademyChatRepository", "Listen failed.", e)
                        return@addSnapshotListener
                    }

                    if (snapshots != null && !snapshots.isEmpty) {
                        val chatListInfo = snapshots.toObjects(ChatListInfo::class.java)
                        chatListInfoLiveData.value = chatListInfo
                    } else {
                        Log.d("StudentToAcademyChatRepository", "Current data: null")
                    }
                }
        }
    ...
    //채팅방 정보 보기에서 학원 정보 불러오기
    fun getAcademyInfo(academyUid : String) : LiveData<AcademyInfo>
    {
        val academyInfo = MutableLiveData<AcademyInfo?>()
        db.collection("academies").document(academyUid).get().addOnSuccessListener { documentSnapshot ->
            Log.d("123123123123",documentSnapshot.toString())
            val info = documentSnapshot.toObject(AcademyInfo::class.java)
            academyInfo.value = info
        }.addOnFailureListener {
            academyInfo.value = null
            Log.e("버그", "StudentToAcademyChatRepository - 선생님 채팅 레포지토리에서 선생님 정보 못 받아오는 중")
        }
