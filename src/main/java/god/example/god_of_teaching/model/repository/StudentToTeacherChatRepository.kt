...
//선생님 채팅 관련 레포지토리
class StudentToTeacherChatRepository@Inject constructor(private val userDataStoreHelper : UserDataStoreHelper) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            db.collection("teacherChatList").document(myUid).collection("other")
                ...
                .addOnFailureListener {
                    Log.d("버그", "StudentToTeacherChatRepository에서 채팅방 못 나가는 중 : ${it.message}")
                }
    ...
    //첫 채팅방 생성
    fun createChatRoom(
        academyUid: String, otherNickname: String,
        message: String, messageTime: String,
        otherChatType: String, myChatType : String)
    {
        CoroutineScope(Dispatchers.IO).launch {
            val myUid = userDataStoreHelper.getUid.first()
            ...
            db.collection("studentChatList").document(academyUid).collection("other")
                .document(myUid)
                .set(teacherChatListInfo)
                .addOnFailureListener { exception ->
                    Log.d("버그", "StudentToTeacherChatRepository에서 상대 선생님 채팅방 생성 못하는 중 : ${exception.message}")
                }
            ...
            db.collection("teacherChatList").document(myUid).collection("other")
                ...
                .set(myChatListInfo)
                .addOnFailureListener { exception ->
                    Log.d("버그", "StudentToTeacherChatRepository에서 내 채팅방 생성 못하는 중 : ${exception.message}")
                }
        }
    }

    //채팅리스트 업데이트
    fun updateChatList(otherUid: String, otherNickname: String,
                ...
                .addOnSuccessListener { document ->
                    if (document.exists()) {
                        ...
                        db.collection("studentChatList").document(otherUid).collection("other")
                            .document(myUid)
                            .set(teacherChatListInfo, SetOptions.merge())
                            .addOnFailureListener { exception ->
                                Log.d("버그", "StudentToTeacherChatRepository에서 상대 선생님 채팅방 생성 못하는 중 : ${exception.message}")
                            }
    ...
    fun changeMessageStatus(otherUid: String) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            db.collection("teacherChatList").document(myUid).collection("other")
                ...
                .set(myChatListInfo, SetOptions.merge())
                .addOnFailureListener { exception ->
                    Log.d("버그", "StudentToTeacherChatRepository에서 신규 메시지중")
                }
        }
    }
    ...
    fun getChatList(): MutableLiveData<List<ChatListInfo>> {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            db.collection("teacherChatList").document(uid)
                .collection("other")
                .orderBy("timeStamp", Query.Direction.DESCENDING)
                .addSnapshotListener { snapshots, e ->
                    if (e != null) {
                        Log.w("StudentToTeacherChatRepository", "Listen failed.", e)
                    ...
                    if (snapshots != null && !snapshots.isEmpty) {
                        ...
                        Log.d("StudentToTeacherChatRepository", "Current data: null")
                    }
                    //Log.d("123123123",chatListInfoLiveData.value.toString())
                }
        }
                    ...
                    if (document.exists()) {
                        db.collection("teacherChatRoom").document(myUid)
                            ...
                            .addOnFailureListener { e ->
                                Log.e("버그", "StudentToTeacherChatRepository - 채팅 전송 못하는 중: ${e.message}")
                            }
                            ...
                            .addOnSuccessListener {
                                db.collection("users").document(otherUid)
                                    ...
                                    .addOnSuccessListener { it ->
                                        ...
                                        sendNotification(otherUid, myNickname, message, serverKey, fcmKey)
                                        it.message?.let { it1 -> Log.d("버그", "StudentToTeacherChatRepository - 알림 차단 목록 못 찾는 중 $it1") }
                                    }
                            }.addOnFailureListener { e ->
                                Log.e("버그", "StudentToTeacherChatRepository - Failed to send message: ${e.message}")
                            }
                    }
                }
                .addOnFailureListener { exception ->
                    Log.d("버그", "StudentToTeacherChatRepository - 블랙리스트 확인 중 오류 발생: ${exception.message}")
                }
        }
    ...
    fun updatePhotoChatRoom(otherUid: String, uris: List<Uri>, messageTime: String, fcmKey: String) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            uris.forEach { uri ->
                ...
                val photoRef = storage.reference.child("teacherChatPhotos/$fileName")
                ...
                photoRef.putFile(uri).addOnSuccessListener {
                    ...
                    Log.e("버그", "StudentToTeacherChatRepository - Failed to upload photo: ${e.message}")
                }
            }
        }
    }
                ...
                if(document.exists())
                {
                        db.collection("teacherChatRoom").document(myUid)
                            ...
                            .addOnSuccessListener {
                                ...
                                Log.e("버그", "StudentToTeacherChatRepository - 사진 전송 못하는 중: ${e.message}")
                            }
                }
                else
                {
                    ...
                    db.collection("studentChatRoom").document(otherUid)
                        ...
                        .addOnSuccessListener {
                            ...
                            db.collection("users").document(otherUid).collection("blacklistNotify").document(myUid).get()
                                ...
                                addOnFailureListener {
                                    it.message?.let { it1 -> Log.d("버그", "StudentToTeacherChatRepository - 알림차단리스트 조회 못하는 중$it1") }
                                }
                        }.addOnFailureListener { e ->
                            Log.e("버그", "StudentToTeacherChatRepository - Failed to send message: ${e.message}")
                        }
    ...
    fun getTeacherInfo(teacherUid : String) : LiveData<TeacherInfo>
    {
        ...
        db.collection("teachers").document(teacherUid).get().addOnSuccessListener { documentSnapshot ->
            ...
            Log.e("버그", "StudentToTeacherChatRepository - 선생님 채팅 레포지토리에서 선생님 정보 못 받아오는 중")
        }
