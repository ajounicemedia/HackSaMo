...
class StudentChatListFragment : Fragment() {
    ...
    private val teacherToStudentChatViewModel : TeacherToStudentChatViewModel by viewModels()
    ...
    override fun onCreateView(
        ...
        teacherToStudentChatViewModel.getChatList()
        ...
        studentChatListAdapter = ChatListAdapter(
            {
                ...
                if(it.myChatType=="teacher")
                {
                    navController.navigate(R.id.teacherToStudentChatRoomFragment, bundle)
                }
                else
                {
                    navController.navigate(R.id.academyToStudentChatRoomFragment,bundle)
                }

                //채팅 리스트 업데이트
                if (it.newMessage == true) {
                    it.otherUid?.let { it1 -> teacherToStudentChatViewModel.changeMessageStatus(it1) }
                }
            }
            ,
            {
    ...
    private fun observeChatMessages() {
        teacherToStudentChatViewModel.chatList?.observe(viewLifecycleOwner, Observer { chatList ->
            //어댑터 다시 생성해야할거 대비
