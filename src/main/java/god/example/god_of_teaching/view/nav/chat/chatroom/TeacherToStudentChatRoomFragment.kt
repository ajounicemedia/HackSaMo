...
@AndroidEntryPoint
class TeacherToStudentChatRoomFragment: Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        otherUid?.let { studentChatView.getMessage(it) }//채팅정보가져오기
        //otherUid?.let { chatView.changeStudentReadCheck(it) }//읽음처리
    ...
    private fun observeChatMessages() {
        studentChatView.studentChatMessages?.observe(viewLifecycleOwner, Observer { messages ->
            //어댑터 다시 생성해야할거 대비
//            if (!::chatMessageAdapter.isInitialized) {
//                createAdapter()
//            }
            //채팅방내에서 새메시지 받을 때 채팅 리스트에서 새로운 메세지 안 보이게하기위해
            otherUid?.let { studentChatView.changeMessageStatus(it) }
...
//                val chatRoomTime = dateFormatChatRoom.format(currentDate)
                        ...
                        studentChatView.createChatRoom(otherUid!!, otherName!!, message,
                            chatListTime, otherChatType!!, myChatType!!
                        )//채팅리스트 업데이트
                        studentChatView.updateChatRoom(
                            otherUid!!,
                            message,
                            chatListTime,
                            fcmKey
...
//                val chatRoomTime = dateFormatChatRoom.format(currentDate)
                        ...
                        val fcmKey = getString(R.string.my_fcm_key)
                        studentChatView.updateChatList(otherUid!!, otherName!!, message, chatListTime)//채팅리스트 업데이트
                        studentChatView.updateChatRoom(otherUid!!, message, chatListTime, fcmKey)//채팅방 업데이트
                        binding.etMessage.text.clear()//전송시 에딧 텍스트 비워줌
...
//        val chatRoomTime = dateFormatChatRoom.format(currentDate)
        ...
        if (uris.isNotEmpty()) {
            if (uris.size > 3) {
                ...
                // 사진 업로드 및 메시지 전송
                val fcmKey = getString(R.string.my_fcm_key)
                studentChatView.sendPhotoMessage(otherUid!!, uris,chatListTime,otherName!!,fcmKey)
            }
        }
    }
        ...
        binding.toolbarChatRoom.setOnMenuItemClickListener { menuItem ->
            when (menuItem.itemId) {
                ...
                R.id.other_info ->
                {
                    ...
                    {
                        chatView.getOtherProfileImage("teachers",otherUid!!)
                    ...
                    if (myChatType=="teacher")//선생님이면 학생정보 보기
                    {
                ...
                R.id.go_out_chatroom-> {//채팅방 나가기
                    studentChatView.goOutChatRoom(otherUid!!)
                    val navController = findNavController()
