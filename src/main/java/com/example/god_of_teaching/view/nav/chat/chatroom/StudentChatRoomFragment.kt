...
@AndroidEntryPoint
class StudentChatRoomFragment: Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        otherUid?.let { studentChatView.getStudentMessage(it) }//채팅정보가져오기
...
//                val chatRoomTime = dateFormatChatRoom.format(currentDate)
                    ...
                    studentChatView.updateStudentChatList(otherUid!!,otherName!!,message,chatListTime)//채팅리스트 업데이트
                    studentChatView.updateStudentChatRoom(otherUid!!,message, chatListTime,fcmKey)//채팅방 업데이트
...
//        val chatRoomTime = dateFormatChatRoom.format(currentDate)
        ...
        if (uris.isNotEmpty()) {
            if (uris.size > 3) {
                ...
                // 사진 업로드 및 메시지 전송
                val fcmKey = getString(R.string.my_fcm_key)
                studentChatView.uploadStudentChatRoomPhotosAndSendMessage(otherUid!!, uris,chatListTime,otherUid!!,otherName!!,fcmKey)
            }
        }
    }
    ...
    private fun createMenu()
    {
        ...
        binding.toolbarChatRoom.setOnMenuItemClickListener { menuItem ->
            when (menuItem.itemId) {
                ...
                R.id.go_out_chatroom->{//채팅방나가기
                    studentChatView.goOutStudentChatRoom(otherUid!!)
                    val navController = findNavController()
