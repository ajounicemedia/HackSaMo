package com.example.god_of_teaching.view.nav.myinfo.registermodify.academy.register
...
import com.example.god_of_teaching.databinding.FragmentTeacherChoiceLocalBinding
import com.example.god_of_teaching.model.`object`.data.LocationData
import com.example.god_of_teaching.view.nav.myinfo.registermodify.adapter.ChoiceBigLocalAdapter
import com.example.god_of_teaching.view.nav.myinfo.registermodify.adapter.ChoiceLocalAdapter
import com.example.god_of_teaching.view.nav.myinfo.registermodify.adapter.MyLocalAdapter
import com.example.god_of_teaching.viewmodel.AcademyViewModel
import dagger.hilt.android.AndroidEntryPoint

//검색 될 주소 찾는 프래그먼트
@AndroidEntryPoint
class AcademySearchedAddressFragment : Fragment() {
    ...
    private fun basicSetting()
    {
        ...
        binding.toolbarTitleChoiceLocal.text = "학원이 속한 구를 선택해주세요\n검색 될 때 사용됩니다"
    ...
    private fun next()//선택한 데이터 업로드 및 다음 프래그먼트로 이동
    {
        binding.btnNextChoiceLocal.setOnClickListener {
            if (myLocalList.isNotEmpty()) {
                ...
                academyView.uploadSearchedLocalList(myLocalList)
                val navController = findNavController()
                navController.popBackStack()
                navController.navigate(R.id.academySubjectFragment)
            } else {
        ...
        bigLocalAdapter = ChoiceBigLocalAdapter(LocationData.bigLocalList) { bigLocalPosition ->
            ...
            localAdapter = ChoiceLocalAdapter(localList[bigLocalPosition]) { localItem ->
                ...
                if(myLocalList.size<3) {
                    ...
                    Toast.makeText(context,"3개를 초과한 지역을 등록할 수 없습니다", Toast.LENGTH_SHORT).show()
                }
