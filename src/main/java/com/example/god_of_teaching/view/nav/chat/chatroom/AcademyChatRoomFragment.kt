...
//학원 채팅 관련 뷰
@AndroidEntryPoint
class AcademyChatRoomFragment : Fragment(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        otherUid?.let { academyChatView.getAcademyMessage(it) }//채팅정보가져오기
    ...
    private fun observeChatMessages() {
        academyChatView.academyChatMessages?.observe(viewLifecycleOwner, Observer { messages ->
            //어댑터 다시 생성해야할거 대비
...
//            }
    ...
    private fun sendMessage()
    {
        ...
        binding.btnSend.setOnClickListener {
            if(otherUid?.let { preferenceUtils.isBlackNotification(it)} == true)
            {
...
//                val chatRoomTime = dateFormatChatRoom.format(currentDate)
                    ...
                    academyChatView.updateAcademyChatList(otherUid!!,otherName!!,message,chatListTime)//채팅리스트 업데이트
                    academyChatView.updateAcademyChatRoom(otherUid!!,message, chatListTime,fcmKey)//채팅방 업데이트
...
//        val chatRoomTime = dateFormatChatRoom.format(currentDate)
        ...
        if (uris.isNotEmpty()) {
            if (uris.size > 3) {
                ...
                // 사진 업로드 및 메시지 전송
                val fcmKey = getString(R.string.my_fcm_key)
                academyChatView.uploadAcademyChatRoomPhotosAndSendMessage(otherUid!!, uris,chatListTime,otherUid!!,otherName!!,fcmKey)
            }
        }
    }
            ...
            when (menuItem.itemId) {
                ...
                R.id.other_info -> {
                    ...
                    viewLifecycleOwner.lifecycleScope.launch {
                        ...
                        {
                            "student"->//학생이면 학원 정보 보이기
                            {
                                val storageReference = Firebase.storage.reference.child("academys/${otherUid}")
                ...
                R.id.go_out_chatroom-> {//채팅방 나가기
                    academyChatView.goOutAcademyChatRoom(otherUid!!)
                    val navController = findNavController()
