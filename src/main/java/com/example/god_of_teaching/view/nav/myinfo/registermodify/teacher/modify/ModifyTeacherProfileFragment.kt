...
import com.example.god_of_teaching.databinding.FragmentManageTeacherProfileBinding
...
//선생님 정보 수정하기 프래그먼트 내 정보에서 uid 꼭!!!!!!!받아와야한다
@AndroidEntryPoint
class ModifyTeacherProfileFragment : Fragment() {
    private val REQUEST_GALLERY = 1001
    private var _binding: FragmentManageTeacherProfileBinding?=null
    ...
    override fun onCreateView(
        ...
        _binding = DataBindingUtil.inflate(inflater, R.layout.fragment_manage_teacher_profile, container, false)
        binding.vm = teacherView
    ...
    private fun basicSetting()
    {
        ...
        ToolbarUtil.setupToolbar(activity as AppCompatActivity, binding.toolbarFixTeacherInfo,"선생님 정보 수정",binding.toolbarTitleFixTeacherInfo)
        NavigationUtil.handleBackPress(this,navController)
    }
    //변경할 항목 선택
    private fun modifyTextInfo()
    {
        //닉네임 수정
//        binding.tvModifyNicknameFixTeacherInfo.setOnClickListener{
//            val navController = findNavController()
//            val bundle = Bundle()
//            bundle.putString("modified_item", "닉네임")
//            navController.navigate(R.id.modifyMyInfoTextFragment, bundle)
//        }
        //한줄 소개 수정
        binding.tvModifyDesFixTeacherInfo.setOnClickListener {
            val navController = findNavController()
            val bundle = Bundle()
        ...
        //학적 상태 수정
        binding.tvModifyStatusFixTeacherInfo.setOnClickListener {
            statusDialog()
        }
        //수업 방식 수정
        binding.tvModifyWayFixTeacherInfo.setOnClickListener {
            wayDialog()
        }
        //활동 지역 수정
        binding.tvModifyLocalFixTeacherInfo.setOnClickListener {
            val navController = findNavController()
            navController.navigate(R.id.modifyTeacherLocalFragment)
        }
        //수업 과목 수정
        binding.tvModifySubjectFixTeacherInfo.setOnClickListener {
            val navController = findNavController()
            navController.navigate(R.id.modifyTeacherSubjectFragment)
        }
        //소개 수정
        binding.tvModifyIntroduceFixTeacherInfo.setOnClickListener {
            val navController = findNavController()
            navController.navigate(R.id.modifyTeacherIntroduceFragment)
        }
    ...
    private fun statusDialog()
    {
        ...
        val statusChoices = arrayOf("휴학", "재학", "졸업")
        ...
        titleTextView.text = "학적 상태"
        builder.setCustomTitle(customTitle)

        builder.setItems(statusChoices) { _, which ->
            val selectedStatus = statusChoices[which]
            teacherView.modifyTeacherInfo("학적 상태", selectedStatus)
        }
    ...
    private fun wayDialog()
    {
        ...
        builder.setItems(wayChoices) { _, which ->
            ...
            teacherView.modifyTeacherInfo("수업 방식", selectedWay)
        }
        val dialog = builder.create()
        dialog.show()
    }
    ...
    {
        binding.ivProfileImgFixTeacherInfo.setOnClickListener {
            val currentDrawable = binding.ivProfileImgFixTeacherInfo.drawable as? BitmapDrawable
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_GALLERY && resultCode == AppCompatActivity.RESULT_OK) {
            ...
            binding.ivProfileImgFixTeacherInfo.tag = "custom"
            resultUri = UCrop.getOutput(data!!)!!
            binding.ivProfileImgFixTeacherInfo.scaleType = ImageView.ScaleType.FIT_CENTER
            binding.ivProfileImgFixTeacherInfo.setImageURI(resultUri)
            binding.ivProfileImgFixTeacherInfo.adjustViewBounds = true
            Log.d("UCropResult", "URI: $resultUri")


            teacherView.uplodeTeacherProfileImage(resultUri!!)
            context?.let { teacherView.uplodeTeacherProfileImageToLocal(resultUri!!, it) }
            //사이즈 조절
...
//            layoutParams.setMargins(100, 100, 100, 100)
    ...
    private fun resetToDefaultImage() {
        binding.ivProfileImgFixTeacherInfo.setImageResource(R.drawable.teacher_profileimage)
        binding.ivProfileImgFixTeacherInfo.tag = "default"
        // 로컬의 이미지 파일 삭제
        // 로컬의 이미지 파일 삭제
        val file = File(context?.filesDir, uid+ "teacher.jpg")
        if (file.exists()) {
            file.delete()
        }

        // ImageView의 속성 조절
        binding.ivProfileImgFixTeacherInfo.scaleType = ImageView.ScaleType.CENTER_CROP
        binding.ivProfileImgFixTeacherInfo.adjustViewBounds = true
            ...
            Log.d("123456기존에 이미지 있나", fileName!!)
                ...
                if (imageFile != null) {
                        if (imageFile.exists()) {
                            ...
                            binding.ivProfileImgFixTeacherInfo.setImageBitmap(loadedBitmap)
                            binding.ivProfileImgFixTeacherInfo.scaleType =
