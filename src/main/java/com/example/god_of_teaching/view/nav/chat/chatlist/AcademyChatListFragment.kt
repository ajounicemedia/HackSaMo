...
class AcademyChatListFragment: Fragment() {
    ...
    override fun onCreateView(
        ...
        academyChatView.getAcademyChatList()
        ...
        academyChatListAdapter = ChatListAdapter(
            {
                ...
                if (it.newMessage == true) {
                    it.otherUid?.let { it1 -> academyChatView.changeMessageStatus(it1) }
                }
            }
            ,
            {
    ...
    private fun observeChatMessages() {
        academyChatView.academyChatList?.observe(viewLifecycleOwner, Observer { chatList ->
            //어댑터 다시 생성해야할거 대비
