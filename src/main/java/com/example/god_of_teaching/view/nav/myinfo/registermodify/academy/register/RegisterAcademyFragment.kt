...
import com.example.god_of_teaching.databinding.FragmentRegisterAcademyBinding
...
class RegisterAcademyFragment : Fragment() {
    ...
    private fun uploadAcademyInfo()
    {
        binding.btnNextRegisterAcademy.setOnClickListener {
            //val nickname = binding.etNicknameRegisterAcademy.text.toString()
            val des = binding.etDesRegisterAcademy.text.toString()

            if (/*des.isEmpty() || nickname.isEmpty() || */classAge == null) {
                Toast.makeText(context, "입력이 안 된 정보가 있습니다.", Toast.LENGTH_SHORT).show()
            } else {
                val navController = findNavController()
//                academyView.uploadAcademyInfo(nickname, des, classAge!!)
//                academyView.uploadAcademyInfoToLocal(nickname, des,classAge!!)
                //닉네임 통일
                academyView.uploadAcademyInfo(des, classAge!!)
                academyView.uploadAcademyInfoToLocal(des,classAge!!)
                if (resultUri != null) {
                    academyView.uplodeProfileImage(resultUri!!)
                    Log.d("12321", resultUri.toString())
                }
