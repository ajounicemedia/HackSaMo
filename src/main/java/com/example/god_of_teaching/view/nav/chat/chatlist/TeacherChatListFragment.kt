...
class TeacherChatListFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        teacherChatView.getTeacherChatList()
        ...
        teacherChatListAdapter = ChatListAdapter(
            {
                ...
                if (it.newMessage == true) {
                    it.otherUid?.let { it1 -> teacherChatView.changeMessageStatus(it1) }
                }
            }
               ,
            {
    ...
    private fun observeChatMessages() {
        teacherChatView.teacherChatList?.observe(viewLifecycleOwner, Observer { chatList ->
            //어댑터 다시 생성해야할거 대비
