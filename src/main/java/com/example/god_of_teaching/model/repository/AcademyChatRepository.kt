...
//학원 채팅 관련 레포지토리
class AcademyChatRepository {
    private val db = Firebase.firestore
    private val storage = Firebase.storage

    //채팅방 나가기
    fun goOutAcademyChatRoom(myUid:String,otherUid: String)
    {
        db.collection("academychatlist").document(myUid).collection("other")
            ...
            .addOnFailureListener {
                // 삭제 실패 시 로직
                Log.d("버그", "AcademyChatRepository에서 채팅방 못 나가는 중 : ${it.message}")
            }
            ...
            "newMessage" to false
                ...
                if (checkBlackList(myUid,otherUid) == true) //내가 블랙리스트면 내것만 업데이트
                {
                    ...
                    db.collection("academychatlist").document(myUid).collection("other")
                        ...
                        .addOnSuccessListener {
                            ...
                            Log.d("버그", "AcademyChatRepository에서 내 채팅방 생성 못하는 중 : ${exception.message}")
                        }
                }
                if (checkBlackList(myUid,otherUid) == false)//블랙리스트 아니면 둘다 업데이트
                {
                    ...
                    db.collection("academychatlist").document(otherUid).collection("other")
                        ...
                        .addOnFailureListener { exception ->
                            Log.d("버그", "AcademyChatRepository에서 상대 선생님 채팅방 생성 못하는 중 : ${exception.message}")
                        }
    ...
    //새로운 메시지인지 확인
    fun changeAcademyMessageStatus(myUid: String, otherUid: String)
    {
        //내 채팅리스트 정보
        val myChatListInfo = hashMapOf(
            "newMessage" to false
        ...
        db.collection("academychatlist").document(myUid).collection("other")
            ...
            .addOnFailureListener { exception ->
                Log.d("버그", "AcademyChatRepository에서 채팅방 설정 못하는 중")
            }
    }

    //선생님 채팅방 목록 가져오기
    fun getAcademyChatList(myUid: String): LiveData<List<ChatListInfo>> {
        val chatListInfoLiveData = MutableLiveData<List<ChatListInfo>>()

        // 'timestamp' 필드를 기준으로 오름차순 정렬
        db.collection("academychatlist").document(myUid)
            .collection("other")
                ...
                if (checkBlackList(myUid,otherUid) == true) //내가 블랙리스트면 내것만 업데이트
                {
                    ...
                    db.collection("academychatroom").document(myUid)
                        ...
                        .addOnFailureListener { e ->
                            Log.d("버그", "AcademyChatRepository에서 상대방 파이어스토어에 데이터 업로드 못하는 중: ", e)
                        }
                }
                if (checkBlackList(myUid,otherUid) == false)//블랙리스트 아니면 둘다 업데이트
                {
                    ...
                    db.collection("academychatroom").document(otherUid)
                        ...
                        .addOnFailureListener { e ->
                            Log.d("버그", "AcademyChatRepository에서 상대방 파이어스토어에 업로드 못하는 중: ", e)
                        }
    ...
    // 채팅 메시지를 가져오는 메서드
    fun getAcademyChatRoom(otherUid: String, myUid: String): LiveData<List<ChatMessageInfo>> {
    ...
    //파이어 스토리지에 사진 업로드
    fun uploadAcademyChatRoomPhotosAndCreateMessage(myUid: String, otherUid: String, uris: List<Uri>, messageTime: String,myNickname:String,serverKey: String, fcmKey: String) {
        uris.forEach { uri ->
            val fileName = UUID.randomUUID().toString()
            val photoRef = storage.reference.child("academy_chat_photos/$fileName")

            photoRef.putFile(uri).addOnSuccessListener {
                photoRef.downloadUrl.addOnSuccessListener  { downloadUrl ->
                    createAcademySinglePhotoMessage(myUid, otherUid, downloadUrl.toString(), messageTime,myNickname,serverKey,fcmKey)
                }
            }.addOnFailureListener { e ->
    ...
    private fun createAcademySinglePhotoMessage(myUid: String, otherUid: String, photoUrl: String, messageTime: String
    ...
    //선생님 정보 가져오기
    fun getAcademyInfo(academyUid : String) : LiveData<AcademyInfo>
    {
        val academyInfo = MutableLiveData<AcademyInfo?>()
        db.collection("academys").document(academyUid).get().addOnSuccessListener { documentSnapshot ->
            val info = documentSnapshot.toObject(AcademyInfo::class.java)
