...
class StudentChatRepository {
    ...
    //채팅방 나가기
    fun goOutStudentChatRoom(myUid:String,otherUid: String)
    {
    ...
    //새로운 메세지인지 확인
    fun changeStudentMessageStatus(myUid: String, otherUid: String)
    {
        //내 채팅리스트 정보
        val myChatListInfo = hashMapOf(
            "newMessage" to false
    ...
    //선생님 채팅방 목록 가져오기
    fun getStudentChatList(myUid: String): LiveData<List<ChatListInfo>> {
        val chatListInfoLiveData = MutableLiveData<List<ChatListInfo>>()

        // 'timestamp' 필드를 기준으로 오름차순 정렬
        db.collection("studentchatlist").document(myUid)
            .collection("other")
        ...
        if (checkBlackList(myUid, otherUid) == true) //내가 블랙리스트면 내것만 업데이트
        {
            ...
            db.collection("studentchatroom").document(myUid)
                ...
                .addOnFailureListener { e ->
                    Log.d("버그", "StudentChatRepository에서 상대방 파이어스토어에 데이터 업로드 못하는 중: ", e)
                }
        }
        if (checkBlackList(myUid, otherUid) == false) //블랙리스트 아니면 둘다 업데이트
        {
            ...
            db.collection("studentchatroom").document(otherUid)
                ...
                .addOnFailureListener { e ->
                    Log.d("버그", "StudentChatRepository에서 상대방 파이어스토어에 업로드 못하는 중: ", e)
                }
        }
    ...
    // 채팅 메시지를 가져오는 메서드
    fun getStudentChatRoom(otherUid: String, myUid: String): LiveData<List<ChatMessageInfo>> {
    ...
    //파이어 스토리지에 사진 업로드
    fun uploadStudentChatRoomPhotosAndCreateMessage(myUid: String, otherUid: String, uris: List<Uri>, messageTime: String,myNickname:String,serverKey: String, fcmKey: String) {
        uris.forEach { uri ->
            val fileName = UUID.randomUUID().toString()
            val photoRef = storage.reference.child("student_chat_photos/$fileName")

            photoRef.putFile(uri).addOnSuccessListener {
                photoRef.downloadUrl.addOnSuccessListener  { downloadUrl ->
                    createStudentSinglePhotoMessage(myUid, otherUid, downloadUrl.toString(), messageTime,myNickname,serverKey,fcmKey)
                }
            }.addOnFailureListener { e ->
    ...
    private fun createStudentSinglePhotoMessage(myUid: String, otherUid: String, photoUrl: String, messageTime: String
    ...
    fun getStudentInfo(studentUid : String) : LiveData<StudentInfo> {
        ...
        db.collection("students").document(studentUid).get()
            ...
            Log.e("버그", "학생 채팅 레포지토리에서 선생님 정보 못 받아오는 중")
        }
